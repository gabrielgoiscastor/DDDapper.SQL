...
namespace Back.Services
{
    ...
    public abstract class BaseService<T> : IDisposable
    {
        ...
        public virtual IEnumerable<T> GetByIds<T>(IEnumerable<string> values, Boolean all = false, String fieldName = "id", Boolean lower = true)
        {
            ParameterValidate(values);

            return Validate(_BaseRepository.GetData<T>(values, all, fieldName, lower));
        }
        ...
        public virtual IEnumerable<T> Get<T>(String value, Boolean all = false, String fieldName = "id", Boolean lower = true)
        {
            ParameterValidate(value);

            return Validate(_BaseRepository.GetData<T>(value, all, fieldName, lower));
        }
        ...
        public virtual IEnumerable<T> GetNot<T>(String value, Boolean all = false, String fieldName = "id", Boolean lower = true)
        {
            ParameterValidate(value);

            return Validate(_BaseRepository.GetDataNot<T>(value, all, fieldName, lower));
        }

        public virtual IEnumerable<T> Get<T>(DateTime value, Boolean all = false, String fieldName = "id")
        {
            ParameterValidate(value);

            return Validate(_BaseRepository.GetData<T>(value, all, fieldName));
        }
        ...
        public virtual IEnumerable<T> Get<T>(IEnumerable<String> values, IEnumerable<String> fieldNames, Boolean combine = false, Boolean all = false)
        {
            if (!ParameterValidate(values) || !ParameterValidate(fieldNames) /*|| fields.Count() != fieldNames.Count()*/)
                throw new BadParameterException();

            return Validate(_BaseRepository.GetData<T>(values, fieldNames, combine, all));
        }

        public virtual IEnumerable<T> GetLike<T>(String value, Boolean all = false, String fieldName = "id", Boolean lower = true)
        {
            ParameterValidate(value);

            return Validate(_BaseRepository.GetDataLike<T>(value, all, fieldName, lower));
        }

        #region Top 1

        public virtual T GetTop1<T>(String value, Boolean all = false, String fieldName = "id", Boolean lower = true)
        {
            ParameterValidate(value);

            return Validate(_BaseRepository.GetDataTop1<T>(value, all, fieldName, lower));
        }
        ...
        public virtual IEnumerable<T> GetByHist<T>(String value, Boolean all = false, String fieldName = "id", Boolean lower = true, Boolean isImportant = false)
        {
            ParameterValidate(value);

            return Validate(_BaseRepository.GetByHist<T>(value, all, fieldName, lower), isImportant);
        }
